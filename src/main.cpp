/*********************************************************************************************************************
 ** 2017-04-24  Built for Sonoff "Switch" based on Homie sample sketch
...
// Globals
 ...
 // Button state
    ...
    byte lastButtonExt = 1;
    byte buttonPressHandled = 0;

  // Sensor consts/vars
    HomieNode switchNode("strip", "switch");
...
void loopHandler() {
  ...
  byte buttonExt = digitalRead(PIN_TRIGGER);

  if ( buttonState != lastButtonState || buttonExt != lastButtonExt ) {
    if (buttonState == LOW || buttonExt == LOW) {
